  ...
  <div id="content">
	...
	<div id="breadcrumb">
        <ul>
			...
			<li>Application company financials</li>
        </ul>
	</div><!--breadcrumb-->
	<div class="inner">
		  <div class="clearer"><!-- --></div>
		  <br /><h2>Manage application company financials</h2><br />
    <div id="sidetabs">
        <ul > 
			...
			<li class="active"><a href="#" title="Financials">Financials</a></li>
		  ...
		  <form name="financialsForm" id="financialsForm" action="/awards/entryapplication/financial.php?code={$applicationData.application_code}" method="post">
			  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="innertable"> 				  
			  <thead>
			  <tr>				
				<th valign="top">Year</th>
				<th valign="top">Gross Revenue</th>
				<th valign="top">Gross Profit</th>
				<th valign="top">Briefly Explain The Drivers of Y/Y Gross Profit Growth or Reduction in each year</th>						
				<th valign="top"></th>	
				<th valign="top"></th>	
			  </tr>
			  </thead>
			  ...
			  <tr>
				...
				<td valign="top"><input type="text" id="revenue_{$item.applicationentity_code}" name="revenue_{$item.applicationentity_code}" value="{$item.applicationentity_revenue}" /></td>				
				<td valign="top"><input type="text" id="profit_{$item.applicationentity_code}" name="profit_{$item.applicationentity_code}" value="{$item.applicationentity_profit}" /></td>
				<td valign="top"><textarea cols="100" rows="10" id="description_{$item.applicationentity_code}" name="description_{$item.applicationentity_code}">{$item.applicationentity_description}</textarea></td>
				<td valign="top"><button type="button" onclick="updateitem('{$item.applicationentity_code}'); return false;">Update</button></td>
			  ...
			  <tr>
				...
				<td valign="top"><input type="text" id="applicationentity_revenue" name="applicationentity_revenue" size="15" /></td>	
				<td valign="top"><input type="text" id="applicationentity_profit" name="applicationentity_profit" size="15" /></td>				
...
function updateitem(id) {
	...
	var revenue 	= $('#revenue_'+id).val();
	var profit 		= $('#profit_'+id).val();
	var description = $('#description_'+id).val();
	
	if(revenue != '' && profit != '' && description != '') {
		if(confirm('Are you sure you want to update this item?')) {
			$.ajax({ 
				type: "GET",
				url: "financial.php?code={/literal}{$applicationData.application_code}{literal}",
				data: "update_code="+id+"&revenue="+revenue+"&profit="+profit+"&description="+description,
				dataType: "json",
