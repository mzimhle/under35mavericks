...
class class_awardquestion extends Zend_Db_Table_Abstract
{
   //declare table variables
	...
	public function changeOrder($section, $code, $newindex)
	{
		...
		$questionData = $this->getByCode($code);
		
		if($questionData) {
			
			if((int)$questionData['awardquestion_index'] == (int)$newindex) {
				/* Nothing is changing. */
				return true;
			} else {
				/* Get subsection by section and index.*/
				$otherquestionData = $this->getByIndex($section, $newindex);
				
				if($otherquestionData) {
					
					/* update with the previous ones index. */
					$data = array();
					$data['awardquestion_index'] = $questionData['awardquestion_index'];
					
					$where		= array();
					$where[]	= $this->getAdapter()->quoteInto('awardquestion_code = ?', $otherquestionData['awardquestion_code']);
					$where[]	= $this->getAdapter()->quoteInto('awardsubsection_code = ?', $section);
					$success	= $this->update($data, $where);
					
					/* Update new sub section with the new order. */
					$data = $where = null; unset($where, $data); 
					$data = array();
					$data['awardquestion_index'] = $newindex;
					
					$where		= array();
					$where[]	= $this->getAdapter()->quoteInto('awardquestion_code = ?', $questionData['awardquestion_code']);
					$where[]	= $this->getAdapter()->quoteInto('awardsubsection_code = ?', $section);
					$success	= $this->update($data, $where);
	...
	public function getByIndex($section, $index)
	{
		$select = $this->_db->select()	
				...
				->where('awardquestion.awardquestion_index = ?', $index)					   
				->limit(1);
	...
	public function getAll() {
		...
		$select = $this->_db->select()	
						...
						->where('awardquestion_deleted = 0 and awardsubsection_deleted = 0')
						->where('awardquestion_active = 1 and awardsubsection_active = 1')
					   ->order(array('awardsubsection.awardsubsection_code desc', 'awardquestion_index'));
	...
	public function getBySubSection($section) {
		...
		$select = $this->_db->select()	
						...
						->where('awardquestion_active = 1 and awardsubsection_active = 1')
						->where('awardquestion.awardsubsection_code = ?', $section)
					   ->order(array('awardquestion_index'));
