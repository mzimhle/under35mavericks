	...
	if(isset($_POST['people_name'])) {
		...
		for($i = 0; $i < count($_POST['people_name']); $i++) {
			...
			if(isset($_POST['people_birthdate'][$i]) && trim($_POST['people_birthdate'][$i]) == '') {
				...
				$errorArray['result'] 			= 0;						
			} else if(date(date('Y-m-d', strtotime(trim($_POST['people_birthdate'][$i])))) != trim($_POST['people_birthdate'][$i])) {
				$errorArray['message'][]	= 'Please add a valid birthdate for the '.($i+1).' director / shareholder';
				$errorArray['result'] 			= 0;				
			}
			/* Designation. */
